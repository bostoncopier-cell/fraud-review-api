...
async def submit(
    ...
    # Combine some text from uploaded files (best-effort; images/PDFs won't decode cleanly)
    combined_text = ""
    for f in files:
...
"""
    ...
    # AI analysis (failsafe so endpoint doesn't crash)
    try:
    ...
    # Email analyst (failsafe so email issues don't crash endpoint)
    try:
        if resend.api_key:
            resend.Emails.send(
                {
